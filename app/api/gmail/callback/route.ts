...
export async function GET(request: Request) {
  ...
  if (error) return redirectWith(`/admin?gmail=denied&reason=${encodeURIComponent(error)}`);

  const code = url.searchParams.get("code");
  const state = url.searchParams.get("state");
  if (!code || !state) return apiError("Missing code or state", 400);
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      ...
      return redirectWith(`/admin/products/${productId}?step=4&gmail=no_refresh_token`);
    }
    if (!tokens.scope?.includes(GMAIL_SCOPE)) {
      return redirectWith(`/admin/products/${productId}?step=4&gmail=missing_scope`);
    }
    ...
    return redirectWith(`/admin/products/${productId}?step=4&gmail=connected`);
  } catch {
    return redirectWith(`/admin/products/${productId}?step=4&gmail=failed`);
  }
}
